...
export function ToastStory() {
  ...
  function fire(variant: ToastVariant, title: string, description: string, action?: string) {
    setToast({ open: false, variant, title, description, action });
    // give Radix a tick to register the close before reopening
    setTimeout(() => setToast({ open: true, variant, title, description, action }), 50);
  }
      ...
      <div className='space-y-10'>
        <StoryGroup title='Variants'>
          <StoryBlock label='default'>
            <Button
              variant='ghost'
              onClick={() => fire('default', 'Heads up!', 'Your session will expire in 5 minutes.')}
            >
              Show default toast
            </Button>
          </StoryBlock>
          <StoryBlock label='success'>
            <Button
              variant='primary'
              onClick={() => fire('success', 'Changes saved', 'Your profile has been updated successfully.', 'Undo')}
            >
              Show success toast
            </Button>
          </StoryBlock>
          <StoryBlock label='error'>
            <Button
              variant='danger'
              onClick={() => fire('error', 'Something went wrong', 'Failed to save changes. Please try again.')}
            >
              Show error toast
            </Button>
          </StoryBlock>
